...
class MarkController extends Controller
{
    ...
    public function selector(MarkSelector $req)
    {
        ...
        if($students->count() < 1){
            return back()->with('pop_error', __('rnf'));
        }
    ...
    public function bulk($class_id = NULL, $section_id = NULL)
    {
        ...
        if($class_id && $section_id){
            ...
            if($st->count() < 1){
                return redirect()->route('marks.bulk')->with('flash_danger', __('srnf'));
            }
            $d['selected'] = true;
    ...
    public function tabulation($exam_id = NULL, $class_id = NULL, $section_id = NULL)
    {
        ...
        if($class_id && $exam_id && $section_id){
            ...
            if(count($sub_ids) < 1 OR count($st_ids) < 1) {
                return Qs::goWithDanger('marks.tabulation', __('srnf'));
            }
    ...
    {
        return redirect()->route('dashboard')->with('flash_danger', __('srnf'));
    }
