...
class UserController extends Controller
{
    ...
    public function reset_pass($id)
    {
        ...
        if(Qs::headSA($id)){
            return back()->with('flash_danger', __('denied'));
        }

        $data['password'] = Hash::make('user');
        $this->user->update($id, $data);
        return back()->with('flash_success', __('pu_reset'));
    }
    ...
    public function store(UserRequest $req)
    {
        ...
        if(!$uname && !$req->email){
            return back()->with('pop_error', __('user_invalid'));
        }
    ...
    public function update(UserRequest $req, $id)
    {
        ...
        // Redirect if Making Changes to Head of Super Admins
        if(Qs::headSA($id)){
            return Qs::json(__('denied'), FALSE);
        }
    ...
    public function destroy($id)
    {
        ...
        // Redirect if Making Changes to Head of Super Admins
        if(Qs::headSA($id)){
            return back()->with('pop_error', __('denied'));
        }

        $user = $this->user->find($id);

        if($user->user_type == 'teacher' && $this->userTeachesSubject($user)) {
            return back()->with('pop_error', __('del_teacher'));
        }
