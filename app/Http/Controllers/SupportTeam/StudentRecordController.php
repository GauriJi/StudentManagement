   ...
   public function __construct(LocationRepo $loc, MyClassRepo $my_class, UserRepo $user, StudentRepo $student)
   {
       ...
       /* $ct = ($ct == 'J') ? 'JSS' : $ct;
        ...
        foreach ($docFields as $doc) {
            if ($req->hasFile($doc)) {
                ...
                $f['name'] = $doc . '.' . $f['ext'];
                $f['path'] = $file->storeAs(Qs::getUploadPath('student').$data['code'], $f['name']);
    ...
    public function show($sr_id)
    {
        ...
        if(Auth::user()->id != $data['sr']->user_id && !Qs::userIsTeamSAT() && !Qs::userIsMyChild($data['sr']->user_id, Auth::user()->id)){
            return redirect(route('dashboard'))->with('pop_error', __('denied'));
        }
    ...
    public function update(StudentRecordUpdate $req, $sr_id)
    {
        ...
        $srec = $req->only(Qs::getStudentData(['aadhar_card', 'prev_marksheet', 'birth_certificate']));
        ...
        foreach ($docFields as $doc) {
            if ($req->hasFile($doc)) {
                ...
                $f = Qs::getFileMetaData($file);
                $f['name'] = $doc . '.' . $f['ext'];
                $f['path'] = $file->storeAs(Qs::getUploadPath('student').$sr->user->code, $f['name']);
                $srec[$doc] = asset('storage/' . $f['path']);
            }
        }
